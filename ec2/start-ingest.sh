...
#instance_info=`aws ec2 describe-instances --region $EC2_REGION --instance-ids $instance_ingest_main `
...
status_code=`aws ec2 describe-instances --region $EC2_REGION --instance-ids $instance_ingest_main | jq ' .Reservations[0] | .Instances[0] | .State | .Code'`
...
echo "MAIN INSTANCE STARTED:$instance_ingest_main"

#TODO: cleanup init file ec2_ingest_init.sh.gz

#VPC instances need allocation-id instead of public-ip
retval=`aws ec2 associate-address --region=$EC2_REGION --instance-id $instance_ingest_main --allocation-id eipalloc-e5d53880`
echo "ASSOCIATE ELASTIC IP ADDRESS RETURNED: $retval"

#Create route to the UCLDC-ingest machine for NAT
create_route_ret=`aws ec2 create-route --route-table-id rtb-7a05f41f --destination-cidr-block 0.0.0.0/0 --instance-id $instance_ingest_main`
echo "CREATE ROUTE RETURNED: $create_route_ret"

#TURN OFF SourceDestCheck so image can function as NAT
make_natable=`aws ec2 modify-instance-attribute --instance-id $instance_ingest_main --no-source-dest-check`
