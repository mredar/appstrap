...
cat user-data/stanbol.sh > ec2_stanbol_init.sh

# only on the t1.micro, tune swap
if [ "$EC2_SIZE" == 't1.micro' ]; then
  cat >> ec2_stanbol_init.sh << DELIM
...
name_cmd="aws ec2 create-tags --region $EC2_REGION --resources ${instance} --tags Key=Name,Value=UCLDC_stanbol"
tags=`$name_cmd`
